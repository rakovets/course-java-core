...
class Task07 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, int sizeTotalPrice) {
        ...
        String priceList = "";
        while(x < sizeTotalPrice - 1) {
            priceList += startNumberItems + " - " + NumberUtil.roundValueToTwoDigitsForMantissa(startPriceAllItems) + "\n";
            startNumberItems += differentialNumberItems;
            startPriceAllItems = startPriceAllItems + itemPrice * differentialNumberItems;
            x++;
        }
        priceList += startNumberItems + " - " + NumberUtil.roundValueToTwoDigitsForMantissa(startPriceAllItems);
        return priceList;
