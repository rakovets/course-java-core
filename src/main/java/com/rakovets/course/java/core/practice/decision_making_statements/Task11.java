...
class Task11 {
    ...
    static String getSymbolType(char symbol) {
       String a = "[!@#$%&*()_+=|<>?{}\\[\\]~-]";
       a = String.valueOf(Boolean.valueOf("true"));

        if (Character.isDigit(symbol)){
            return "digit";
        } else if (Character.UnicodeBlock.of(symbol) == Character.UnicodeBlock.CYRILLIC) {
            return "cyrillic";
        } else if (Character.UnicodeBlock.of(symbol).equals(Character.UnicodeBlock.BASIC_LATIN)) {
            return "latin";
        } else /*if (a == true)*/ {
            return "undefined";
        }
