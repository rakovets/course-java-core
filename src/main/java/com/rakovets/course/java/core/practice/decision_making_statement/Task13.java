...
class Task13 {
    ...
    static int getFloorNumber(int numberFloors, int numberApartmentsPerFloor, int apartmentNumber) {
        double floorNumber = apartmentNumber % (numberApartmentsPerFloor * numberFloors) / (double)numberApartmentsPerFloor;
        return (int)(floorNumber > 0 ? floorNumber + 1 : numberFloors);
    }
}
