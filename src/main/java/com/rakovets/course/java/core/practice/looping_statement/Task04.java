...
class Task04 {
    ...
    static int calculateHealthPointsByTime(int healthPoints, double regenerationPercentFromCurrentHealth, int hoursAfterRespawn) {
       int hour = 1;
       double fractionalHealthPoints = healthPoints;
        double percentHealth = regenerationPercentFromCurrentHealth / 100;

       for ( ; hour <= hoursAfterRespawn; hour++) {
           fractionalHealthPoints = fractionalHealthPoints + fractionalHealthPoints * percentHealth;
       }
       return (int) fractionalHealthPoints;
    }
}
