...
class Task03 {
    ...
    static double getTotalDepositAmount(double depositAmount, double annualDepositPercent, int depositTerm) {
        ...
        for (int i = 1; i <= depositTerm; i++) {
            capitalization = depositAmount * annualDepositPercent * 0.01;
            depositAmount += capitalization;
        }
        depositAmount = BigDecimal.valueOf(depositAmount).setScale(2,RoundingMode.HALF_UP).doubleValue();
