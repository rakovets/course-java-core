...
class Task03 {
    ...
    static double getTotalDepositAmount(double depositAmount, double annualDepositPercent, int depositTerm) {
        double totalDepositAmout = depositAmount;

        for (int i = 1; i <= depositTerm; i++) {
            totalDepositAmout += totalDepositAmout * (annualDepositPercent / 100);
        }
        totalDepositAmout = BigDecimal.valueOf(totalDepositAmout).setScale(2, RoundingMode.HALF_UP).doubleValue();

        return totalDepositAmout;
    }
}
