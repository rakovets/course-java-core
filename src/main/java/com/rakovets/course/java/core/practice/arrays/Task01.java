...
class Task01 {
    ...
    static double getAverageMark(int[] marks) {
        ...
        double sumMarks = 0;
        for (int i : marks) {
            sumMarks += i;   //находим сумму всех элементов массива
        }
        averageMarks = NumberUtil.roundValueToTwoDigitsForMantissa(sumMarks / marks.length);
        return averageMarks;
    }
    ...
    static int getMinMark(int[] marks) {
        ...
        for (int j : marks) {
            if(minMarks > j) {  //находим минимальный элемент
                minMarks = j;
            }
        }
        return minMarks;
    ...
    static int getMaxMark(int[] marks) {
        int MaxMark = marks[0];
        for (int k : marks) {
            if(MaxMark < k) {  //находим максимальный элемент
                MaxMark = k;
            }
        }
        return MaxMark;
