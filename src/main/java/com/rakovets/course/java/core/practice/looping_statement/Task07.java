...
class Task07 {
    ...
    static double calculateTotalDistance(int startDistance, int finishDistance, double dailyProgressAsPercentage) {
        ...
        for (int i = 1; currentdistance <= finishDistance; i++) {
            if (currentdistance == 0)
                return 0;
            currentdistance += currentdistance / 100 * dailyProgressAsPercentage;
            alldistance = alldistance + currentdistance;
        }
        double totalDistance = alldistance;
