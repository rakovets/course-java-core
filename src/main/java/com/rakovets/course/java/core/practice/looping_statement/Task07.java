...
class Task07 {
    ...
    static double calculateTotalDistance(int startDistance, int finishDistance, double dailyProgressAsPercentage) {
        ...
        double daylyDistance = startDistance;
        for (int i = 1;daylyDistance <= finishDistance && startDistance !=0 ;i++) {
            daylyDistance += daylyDistance * dailyProgressAsPercentage * 0.01;
             totalDistance += daylyDistance;
        }
