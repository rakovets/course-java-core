...
class Task05 {
    ...
    static int calculateRaidTime(int healthPoints, double regenerationPercentPerHour, int averageDamagePerHour) {
        int hours = 0;
        double currentHealthPoint = healthPoints;

        while (currentHealthPoint > 0.0) {
            currentHealthPoint = (currentHealthPoint - averageDamagePerHour) + currentHealthPoint * regenerationPercentPerHour / 100;
            hours++;
