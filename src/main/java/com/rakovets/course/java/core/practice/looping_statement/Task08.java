...
class Task08 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, int sizeTotalPrice) {
        String pricesList = startNumberItems + " - " + startPriceAllItems;
        ...
        for (int i = 1; i < sizeTotalPrice; i++) {
            ...
            double totalPrice = BigDecimal.valueOf(itemPrice * startNumberItems).setScale(2, RoundingMode.HALF_UP).doubleValue();
            pricesList += "\n" + startNumberItems + " - " + totalPrice;
        }
        return pricesList;
    }
}
