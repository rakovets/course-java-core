...
class Task08 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, double differentialSell, int sizeTotalPrice) {
        ...
        totalPriceList += startNumberItems + " - " + startPriceAllItems + " with sell " + "0.0%";
        for (int i = 1; i < sizeTotalPrice; i++) {
            int numberItem = differentialNumberItems * i + startNumberItems;
            double priceAllItems = startPriceAllItems / startNumberItems * numberItem;
            double sale = i * priceAllItems * differentialSell / 100;
            double priceWithSale = NumberUtil.roundValueToTwoDigitsForMantissa(priceAllItems - sale);
            totalPriceList += "\n" + numberItem + " - " + priceWithSale + " with sell " + i * differentialSell + "%";
        }
