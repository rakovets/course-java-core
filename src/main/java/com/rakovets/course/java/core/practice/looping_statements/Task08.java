...
class Task08 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, double differentialSell, int sizeTotalPrice) {
        ...
        double sell = 0.0;
        for(int i = 0, currentNumberItem = startNumberItems;
            i < sizeTotalPrice;
            i++, currentNumberItem += differentialNumberItems, sell += differentialSell) {
            double priceWithSell = ( pricePerItem * currentNumberItem) - (pricePerItem * currentNumberItem * sell / PROCENT);
            totalPriceList += currentNumberItem + " - "
                    + NumberUtil.roundValueToTwoDigitsForMantissa(priceWithSell)
                    + " with sell " + sell + "%";
