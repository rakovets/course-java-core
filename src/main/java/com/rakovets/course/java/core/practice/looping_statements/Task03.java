...
class Task03 {
    ...
    static int calculateHealthPointsByTime(int healthPoints, double regenerationPercentFromCurrentHealth, int hoursAfterRespawn) {
        ...
        for (int i = 0; i < hoursAfterRespawn; i++ ) {
            double regeneratedHealthPoint = raidBossHealthPoint * (regenerationPercentFromCurrentHealth / 100.0);
            raidBossHealthPoint += regeneratedHealthPoint;
        }
        return (int) raidBossHealthPoint;
    }
}
