...
class Task03 {
    ...
    static int calculateHealthPointsByTime(int healthPoints, double regenerationPercentFromCurrentHealth, int hoursAfterRespawn) {
        double countHealthPoints = healthPoints;
        double regenHealth;

        for (int i = 0; i < hoursAfterRespawn; i++) {
            regenHealth = countHealthPoints * (regenerationPercentFromCurrentHealth / 100);
            countHealthPoints += regenHealth;
        }
        return (int) countHealthPoints;
    }
}
