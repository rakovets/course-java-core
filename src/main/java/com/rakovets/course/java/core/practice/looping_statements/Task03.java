...
class Task03 {
    ...
    static int calculateHealthPointsByTime(int healthPoints, double regenerationPercentFromCurrentHealth, int hoursAfterRespawn) {
        int respawnOurCount = 0;
        double currentBossHealthPoints = healthPoints;

        while (respawnOurCount != hoursAfterRespawn) {
            currentBossHealthPoints +=  currentBossHealthPoints * (regenerationPercentFromCurrentHealth / 100);
            respawnOurCount++;
        }
        int currentBossHealthPointsRounded = (int) currentBossHealthPoints;

        return currentBossHealthPointsRounded;
    }
}
