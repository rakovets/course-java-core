...
class Task04 {
    ...
    static int calculateHealthPointsByTime(int healthPoints, double regenerationPercentFromCurrentHealth, int hoursAfterRespawn) {
        ...
        double healthPointsAfterStart = healthPoints;
        for (int i = 1; i <= hoursAfterRespawn; i++) {
            healthPointsAfterStart += healthPointsAfterStart * regenerationPercentFromCurrentHealth / 100;
        }
        int result = (int)healthPointsAfterStart;
        return result;
    }
}
