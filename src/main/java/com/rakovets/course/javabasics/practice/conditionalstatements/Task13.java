...
public class Task13 {
    ...
    static int getFloorNumber(int numberFloors, int numberApartmentsPerFloor, int apartmentNumber) {
        ...
        if (apartmentNumber <= porch && apartmentNumber % numberApartmentsPerFloor > 0)  {
            return  (apartmentNumber / numberApartmentsPerFloor) + 1 ;
        } else if (apartmentNumber <= porch && apartmentNumber % numberApartmentsPerFloor == 0)  {
            return  (apartmentNumber / numberApartmentsPerFloor);
        } else   {
            return  ((apartmentNumber - 1) % porch) / (numberApartmentsPerFloor) + 1;
        }
    }
}
